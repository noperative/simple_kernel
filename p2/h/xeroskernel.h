...
#define STATE_SLEPT     2
#define STATE_SEND      3
#define STATE_RECV      4
#define STATE_RECV_ANY  5
...
struct struct_pcb {
  ...
  int             ret;    /* Return value of system call             */
                          ...
                          /* call                                    */
  long            args;
  int             ticks; /* time left to sleep on the sleep queue*/
  pcb*            send_queue; /* processes waiting to send to this pcb*/
  pcb*            recv_queue; /* processes waiting to recv to this pcb*/
  msg_buffer      buffer; /* buffer stuff */
};
...
void     root( void );
/* The idle process that gets called when no other processes are running */
void     idleproc( void );
